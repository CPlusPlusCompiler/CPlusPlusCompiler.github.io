...
var symbols = require('react-async-ssr/symbols');
var isPromise = _interopDefault(require('is-promise'));
var constants = require('./constants.js');
var shared = require('./shared.js');
var _extends = _interopDefault(require('@babel/runtime/helpers/extends'));
var isClassComponent = _interopDefault(require('is-class-component'));
...
var Resource = /*#__PURE__*/function () {
  function Resource(factory, req, cacheKey, context, parent) {
    ...
    promise[symbols.ABORT] = this.dispose.bind(this);
    promise[symbols.ON_MOUNT] = this._mounted.bind(this);
    if (factory._noSsr) promise[symbols.NO_SSR] = true;
    this._value = promise;
  }
  ...
  _proto._load = function _load() {
    ...
    !isPromise(promise) ?  invariant(false, "Fetch function must return a promise - got " + promise)  : void 0; // Set loading status

    this._status = LOADING; // Record promise's abort handler if defined

    var abort = promise.abort;
    if (isItType.isFunction(abort)) this._abort = abort.bind(promise); // Update status when promise resolves
...
Resource.prototype[constants.IS_RESOURCE] = true;
...
var ResourceFactory = /*#__PURE__*/function () {
  function ResourceFactory(fetchFn, options) {
    ...
    if (options != null) {
      shared.validateOptions(options);
      id = options.id;
      ...
      if (id == null) {
        ...
        !isItType.isFullString(id) ?  invariant(false, "options.id must be a non-empty string if provided - got " + id)  : void 0; // Using global cache implies serialization
        ...
        cacheVar = shared.getCacheVarFromOptions(options);
      ...
      if (serialize == null || serialize === false) {
        ...
        !isItType.isFunction(serialize) ?  invariant(false, "options.serialize must be a function or boolean if provided - got " + serialize)  : void 0;
      ...
      if (noSsr == null) {
        ...
        !isItType.isBoolean(noSsr) ?  invariant(false, "options.noSsr must be a boolean if provided - got " + noSsr)  : void 0; // No-SSR mode is only enabled on server side
  ...
  _proto.use = function use(req) {
    ...
    if (IS_NODE) {
      var context = react.useContext(shared.ServerContext);
...
function wrapSerializer(serialize) {
  return function (req) {
    ...
    !isItType.isString(cacheKey) ?  invariant(false, "serialize() must return a string - got " + cacheKey)  : void 0;
    return cacheKey;
  };
}
...
function isResource(value) {
  return value != null && !!value[constants.IS_RESOURCE] && isItType.isFunction(value.read);
}

function withResources(Component) {
  !isItType.isFunction(Component) ?  invariant(false, "withResources() must be passed a React component - got " + Component)  : void 0;
...
function preloadData(options) {
  return new Promise(function (resolve) {
    ...
    var cacheVar = shared.getCacheVarFromOptionsWithValidate(options);
